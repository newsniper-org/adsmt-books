...
adsmt에서는 벤더링된 OxiZ 백엔드가 이 합동-인식 매칭을
_기본값_으로 삼으며, _CCFV_ — 자유 변수를 갖는 합동 폐포
(congruence closure with free variables) — 라 불리는 하나의
통합된 코어로 실현된다. 이는 Barbosa–Fontaine–Reynolds의
E-기저 (비)단일화를 따른 것이다. 트리거는 기저 합동을
_modulo_하여 매치된다: 패턴 $f(g(x))$는, $c$와 $g(a)$가
합동일 때마다 기저 항 $f(c)$에 대해 발화하며, 이것이 바로
평평한 우주를 훑는 구문적 순회가 놓치는 매치이다.
모델 완성 엔진 — 모델을 구성하여 `sat`을 답할 수 있는
엔진 — 에게 이는 완전성의 이득일 뿐 아니라 _건전성_
요구사항이다: 그러한 합동 매치를 놓치면 엔진이 사실은
`unsat`인 문제에 대해 합동을 보지 못하는 모델을 `sat`으로
인증하게 된다. 같은 CCFV 코어는 충돌-주도 인스턴스화와
모델 완성 탐색도 표현하며, 차이는 오직 해결하는 제약과
실제 합동을 읽는지 아니면 기본값으로 확장된 전체 관점을
읽는지에만 있다.
