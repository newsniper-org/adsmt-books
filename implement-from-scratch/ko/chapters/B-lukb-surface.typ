...
표면의 `=`/`!=`와 비교는 `Eq`/`Ord`/`UpCast` 관계 계열이
*게이트*한다 (Rust식 `Eq`-게이팅): 선언된 모든 소트는
빌트인 `Eq`를 자동 부여받고, `data` 선언의 `Eq`는
*합법적으로* 수용되며 (엔진이 동치 + 결정성 법칙을 방증),
교차-소트 숫자 등식과 비교는 주입을 수행하는 `UpCast`를
premise로 명명하는 `PartialEq`/`PartialOrd` 인스턴스를
해석한다. verus가 방출하는 `is-{ctor}` tester 호출은
