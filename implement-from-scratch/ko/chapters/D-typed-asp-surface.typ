...
#table(
  ...
  [L3], [완전 안정 모델 + loop-formula 인증서], [계획(하드게이트)],
  [L4], [선택 `{…}` / 분리 머리], [계획],
  [L5], [집계 + 약 제약 / 최적화], [계획],
  [L6], [1급 귀추], [규칙 IR에 병합됨],
)
...
그래프를 만들어 음의 사이클을 거부하고
(`FaceError::NonStratified`), solver는 계층을 아래에서
위로 평가하므로 모든 `not`은 이미 결정된 하위 계층을
읽는다. `not` 아래의 모든 변수는 양의 본문 원자로도 묶여야
한다(자유 변수는 닫힌 세계 부정이 아니라 전칭이 된다).
...
의도적이다: 비계층화 프로그램, 안전하지 않은 규칙(미묶임
변수), 무한 도메인, 인증서 없는 임의의 "답 집합 없음"
주장은 근사가 아니라 `FaceError`나 `Unknown`이다.
