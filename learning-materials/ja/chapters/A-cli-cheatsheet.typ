...
候補ごとの `check-sat` は最上位の `(check-sat)` と*同じ完全な
求解経路* — OxiZ 委譲を含む — を通る。よって理論検索は公理化
された関数 (`Add`、`Poly` … 実際の検証器の義務が使う符号化で、
ネイティブエンジンは `unknown` だが e-matching / MBQI が解消する)
の背後の目標まで推論する。ネイティブ算術に限定されない。
