...
정제된 화살표의 후행조건이 잎을 공급하고, 다리가 그것을
`preserving(f)` 에 묶는다. 일반적 `'p` 는 `keeps_pos` 를
_한 번_, 추상적으로 검사한다는 뜻이며, 나중에 당신이
넘기는 모든 구체적 서술어 — 양수성, 유계성, 불변식 — 가
그 단 하나의 검사된 증명을 재사용한다.
